...
import { loadConfig, quoteWildcardStars } from "./config.js";
...
`;
  ...
  // Without this pre-pass js-yaml would throw "unidentified alias" because a
  // leading `*` is the YAML alias-reference prefix.
  it("accepts unquoted *@domain.com wildcards in admins and permissions", () => {
    const yaml = `
ci_providers:
  - name: default
    type: gitlab
...
`;
    vi.mocked(readFileSync).mockReturnValue(yaml);
    const config = loadConfig("/fake.yml");
    expect(config.admins).toEqual(["*@kuehne-nagel.com"]);
    expect(config.permissions[0].users).toEqual(["*@kuehne-nagel.com", "*"]);
  });
});

describe("quoteWildcardStars", () => {
  it("quotes a bare `- *@domain.com` list item", () => {
    expect(quoteWildcardStars("admins:\n  - *@co.com\n"))
      .toBe('admins:\n  - "*@co.com"\n');
  });

  it("quotes a bare `- *` catch-all", () => {
    expect(quoteWildcardStars("admins:\n  - *\n"))
      .toBe('admins:\n  - "*"\n');
  });

  it("quotes `key: *foo` in mapping value position", () => {
    expect(quoteWildcardStars("pattern: *@co.com\n"))
      .toBe('pattern: "*@co.com"\n');
  });

  it("leaves already-quoted values alone", () => {
    const input = 'admins:\n  - "*@co.com"\n  - \'*\'\n';
    expect(quoteWildcardStars(input)).toBe(input);
  });

  it("leaves YAML anchor declarations (`&name`) alone", () => {
    // `&` is a YAML anchor, not part of our wildcard syntax — must pass through.
    const input = "shared: &shared\n  foo: bar\n";
    expect(quoteWildcardStars(input)).toBe(input);
  });

  it("doesn't touch text inside other contexts (no `-` or `:` prefix)", () => {
    expect(quoteWildcardStars("# free *@foo.com text in a comment\n"))
      .toBe("# free *@foo.com text in a comment\n");
  });
});

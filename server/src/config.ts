          ...
          items: {
            ...
            properties: {
              ...
              users: {
                ...
                items: {
                  type: "object",
                  required: ["email", "password_sha256"],
                  properties: {
                    email: { type: "string" },
                    password_sha256: { type: "string" },
...
/**
 * Pre-process YAML so wildcard patterns like `*` and `*@domain.com` (used in
 * `admins:`, `permissions[].users`, `allowed_users:`) don't trip js-yaml's
 * alias parser. YAML treats a leading `*` as an alias reference — so an
 * unquoted `- *@kuehne-nagel.com` fails with "unidentified alias".
 *
 * We auto-quote `*`-prefixed tokens that appear in list-item (`- *...`) or
 * mapping-value (`: *...`) position. Already-quoted values are left alone
 * (the leading `"` or `'` prevents the regex from matching). The repo
 * doesn't use real YAML aliases anywhere, so this is safe in practice.
 */
export function quoteWildcardStars(raw: string): string {
  return raw.replace(
    /(^[ \t]*-[ \t]+|:[ \t]+)(\*[^\s#"'{}\[\],&]*)/gm,
    '$1"$2"',
  );
...
export function loadConfig(configPath?: string): AppConfig {
  ...
  const parsed = yaml.load(quoteWildcardStars(raw)) as Record<string, unknown>;

...
import { hasLoginAccess } from "../permissions.js";
...
export class LocalProvider implements AuthProvider {
  ...
  setupRoutes(router: Router): void {
    ...
    router.post("/api/auth/local/login", (req, res) => {
      ...
      const matchedUser = (this.config.users ?? []).find(
        (u) => u.email === email && u.password_sha256 === submittedHash
      );

      if (!matchedUser) {
        access({ email, provider: "local" }, "login_failed", { reason: "invalid_credentials" });
      ...
      // Check permissions
      if (!hasLoginAccess(user, this.appConfig)) {
        access(user, "login_failed", { reason: "access_denied" });

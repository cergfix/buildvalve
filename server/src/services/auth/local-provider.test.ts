...
function makeLocalConfig(overrides: Partial<LocalProviderConfig> = {}): LocalProviderConfig {
  return {
    ...
    label: "Local Login",
    users: [
      { email: "alice@company.com", password_sha256: sha256("secret123"), groups: ["devops"] },
      { email: "bob@company.com", password_sha256: sha256("hashed-pw"), groups: [] },
    ],
    ...overrides,
  };
}
...
describe("LocalProvider", () => {
  ...
  describe("POST /api/auth/local/login", () => {
    ...
    it("authenticates user with correct password (hashed)", async () => {
      const { handler } = await importAndSetup();
    ...
    it("authenticates user with correct hashed password (second user)", async () => {
      const { handler } = await importAndSetup();

...
describe("GitLabProvider", () => {
  ...
  describe("cancelPipeline", () => {
    it("POSTs to /projects/:id/pipelines/:pid/cancel", async () => {
      fetchSpy.mockResolvedValueOnce(
        new Response(JSON.stringify({ id: 42, status: "canceled" }), { status: 200 }),
      );

      await provider.cancelPipeline("1", "42");

      const [url, opts] = fetchSpy.mock.calls[0] as [string, RequestInit];
      expect(url).toBe("https://gitlab.example.com/api/v4/projects/1/pipelines/42/cancel");

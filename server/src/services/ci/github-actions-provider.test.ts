...
describe("GitHubActionsProvider", () => {
  ...
  describe("cancelPipeline", () => {
    it("POSTs to /runs/:id/cancel and resolves on 202", async () => {
      fetchSpy.mockResolvedValueOnce(new Response("", { status: 202 }));
      await expect(provider.cancelPipeline("o/r", "12345")).resolves.toBeUndefined();
      const [url, opts] = fetchSpy.mock.calls[0] as [string, RequestInit];
      expect(url).toBe("https://api.github.com/repos/o/r/actions/runs/12345/cancel");
      expect(opts.method).toBe("POST");
    });

    it("treats 409 (already-completed run) as a no-op", async () => {
      fetchSpy.mockResolvedValueOnce(new Response("run already completed", { status: 409 }));
      await expect(provider.cancelPipeline("o/r", "12345")).resolves.toBeUndefined();
    });

    it("throws CIProviderError on other non-OK statuses", async () => {
      fetchSpy.mockResolvedValueOnce(new Response("nope", { status: 500 }));
      await expect(provider.cancelPipeline("o/r", "12345")).rejects.toThrow(/github-actions API error 500/);
    });
  });
});

...
describe("CircleCIProvider", () => {
  ...
  describe("getJobTrace", () => {
    ...
    it("returns empty string when no steps available — keeps SSE stream open for queued jobs", async () => {
      // CircleCI's v1.1 endpoint returns `steps: []` for jobs whose executor
      // hasn't started yet. Returning empty (instead of a placeholder)
      // means the SSE poller won't send misleading content and won't
      // mark the stream as "having output" prematurely.
      fetchSpy.mockResolvedValueOnce(
    ...
    it("returns fallback message on API error", async () => {
      ...
      const result = await provider.getJobTrace("gh/org/repo", "100");
      expect(result).toMatch(/HTTP 404|web UI/i);
    });

    it("falls back to raw text body when output_url returns non-JSON", async () => {
      fetchSpy.mockResolvedValueOnce(
        new Response(
          JSON.stringify({
            steps: [
      ...
      fetchSpy.mockResolvedValueOnce(new Response("plain text log line\n", { status: 200 }));
  ...
  describe("status normalization", () => {
    it("normalizes CircleCI-specific statuses", async () => {
      const statuses = [
        ...
        // CircleCI's "waiting" / "not yet started" job statuses — every one
        // must normalize to "pending" so the SSE log stream isn't closed
        // before the executor spins up.
        { input: "not_running", expected: "pending" },
        { input: "blocked", expected: "pending" },
        { input: "retried", expected: "canceled" },
        { input: "terminated-unknown", expected: "failed" },
        { input: "unauthorized", expected: "failed" },
        // Future/unknown statuses must default to pending, not pass-through —
        // an unrecognized status used to close the SSE stream.
        { input: "some_new_status_circleci_invents_later", expected: "pending" },
  ...
  describe("cancelPipeline", () => {
    it("cancels every non-terminal workflow in the pipeline", async () => {
      // 1st call: list workflows
      fetchSpy.mockResolvedValueOnce(
        new Response(
          JSON.stringify({
            items: [
              { id: "wf-1", name: "build", status: "running", created_at: "...", stopped_at: null },
              { id: "wf-2", name: "deploy", status: "success", created_at: "...", stopped_at: null },
              { id: "wf-3", name: "verify", status: "on_hold", created_at: "...", stopped_at: null },
            ],
          }),
          { status: 200 },
        ),
      );
      // 2nd + 3rd: cancel wf-1 and wf-3 (wf-2 is terminal)
      fetchSpy.mockResolvedValueOnce(new Response("", { status: 202 }));
      fetchSpy.mockResolvedValueOnce(new Response("", { status: 202 }));

      await provider.cancelPipeline("gh/org/repo", "pipe-uuid");

      const cancelUrls = fetchSpy.mock.calls.slice(1).map((c: any) => c[0] as string);
      expect(cancelUrls).toContain("https://circleci.com/api/v2/workflow/wf-1/cancel");
      expect(cancelUrls).toContain("https://circleci.com/api/v2/workflow/wf-3/cancel");
      expect(cancelUrls.some((u: string) => u.includes("wf-2"))).toBe(false);
    });

    it("treats 'workflow not running' (4xx) as idempotent no-op, not an error", async () => {
      fetchSpy.mockResolvedValueOnce(
        new Response(
          JSON.stringify({
            items: [{ id: "wf-1", name: "build", status: "running", created_at: "...", stopped_at: null }],
          }),
          { status: 200 },
        ),
      );
      fetchSpy.mockResolvedValueOnce(new Response("workflow is not running", { status: 400 }));

      // Should not throw — race between status check and cancel is a no-op.
      await expect(provider.cancelPipeline("gh/org/repo", "pipe-uuid")).resolves.toBeUndefined();
    });

    it("throws CIProviderError on unexpected upstream failure (e.g. 500)", async () => {
      fetchSpy.mockResolvedValueOnce(
        new Response(
          JSON.stringify({
            items: [{ id: "wf-1", name: "build", status: "running", created_at: "...", stopped_at: null }],
      ...
      await expect(provider.cancelPipeline("gh/org/repo", "pipe-uuid")).rejects.toThrow(/circleci API error 500/);
    });
  });
});

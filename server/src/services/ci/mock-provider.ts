...
export class MockCIProvider implements CIProvider {
  ...
  async cancelPipeline(_projectId: string, pipelineId: string): Promise<void> {
    const p = pipelinesDB.get(pipelineId);
    if (!p) throw new CIProviderError(404, "Mock pipeline not found", this.name, `/pipelines/${pipelineId}/cancel`);
    p.status = "canceled";
    p.updated_at = new Date().toISOString();
    const jobs = jobsDB.get(pipelineId) ?? [];
    for (const j of jobs) {
    ...
    logger.info(`Mock Pipeline ${pipelineId} (${this.type}) canceled`);
  }

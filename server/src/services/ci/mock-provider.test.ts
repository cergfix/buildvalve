...
describe("MockCIProvider", () => {
  ...
  describe("cancelPipeline", () => {
    it("flips the pipeline + its in-flight jobs to canceled", async () => {
      const p = new MockCIProvider("test", "gitlab");
      const triggered = await p.triggerPipeline("repo", "main", []);

      await p.cancelPipeline("repo", triggered.id);

      const after = await p.getPipeline("repo", triggered.id);
      expect(after.status).toBe("canceled");
      const jobs = await p.getPipelineJobs("repo", triggered.id);
      // The "running" job should now be canceled; the already-success one is unchanged.
      expect(jobs.find((j) => j.name === "deploy")!.status).toBe("canceled");
      expect(jobs.find((j) => j.name === "build")!.status).toBe("success");
    });

    it("throws 404 when canceling an unknown pipeline", async () => {
      const p = new MockCIProvider("test", "gitlab");
      await expect(p.cancelPipeline("repo", "does-not-exist")).rejects.toThrow(/Mock pipeline not found/);
    });
  });
});

...
export class GitHubActionsProvider implements CIProvider {
  ...
  async cancelPipeline(projectId: string, pipelineId: string): Promise<void> {
    // POST returns 202 Accepted (no body) on success, 409 if already
    // completed — treat 409 as a no-op so cancel is idempotent.
    const url = `${this.apiUrl}/repos/${projectId}/actions/runs/${pipelineId}/cancel`;
    const res = await fetch(url, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${this.token}`,
    ...
    if (!res.ok && res.status !== 409) {
      const text = await res.text();
      throw new CIProviderError(res.status, text, "github-actions", `/repos/${projectId}/actions/runs/${pipelineId}/cancel`);
    }
  }

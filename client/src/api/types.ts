...
export interface PipelineHistoryEntry {
  ...
  duration: number | null;
  provider?: CIProviderType;
  /** Email of the user who triggered this run via BuildValve (only set for BV-triggered runs). */
  triggered_by?: string;
  /** Variables submitted when this run was triggered (only set for BV-triggered runs). */
  triggered_variables?: Record<string, string>;
}
...
export interface ResolvedPipeline {
  ...
  ref: string;
  workflow_id?: string;
  variables: { key: string; value: string; locked: boolean; description?: string; type?: string; options?: string[] }[];
  provider?: string;
  external_id?: string;
